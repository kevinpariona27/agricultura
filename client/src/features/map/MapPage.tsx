...
interface ParcelMarker { id: number; name: string; area: number; location: string; coords: [number, number]; }
interface DrawnPolygon { latlngs: [number, number][]; area: number; }

function CenterOnMarkers({ markers }: { markers: ParcelMarker[] }) {
  const map = useMap();
  useEffect(() => {
    if (markers.length === 0) return;
    if (markers.length === 1) map.setView(markers[0].coords, 13);
    else {
...
function DrawControl({ onPolygonDrawn }: { onPolygonDrawn: (poly: DrawnPolygon) => void }) {
  ...
  useEffect(() => {
    ...
    pm.setGlobalOptions({ snappable: false, allowSelfIntersection: false });
    ...
    map.on("pm:create", (e: any) => {
      ...
      const areaHa = Math.round(areaSqM / 100) / 10;
...
export function MapPage() {
  ...
  const handleSaveParcel = async () => {
    ...
    try {
      ...
      await create({ name: parcelName.trim(), area: drawnPolygon.area, location: `Lat ${avgLat.toFixed(4)}, Lng ${avgLng.toFixed(4)}`, soil_type: "franco" });
  ...
  if (loading) return <div className="rounded-2xl border border-dashed border-border py-12 text-center text-muted-foreground">Cargando...</div>;
    ...
    <div>
      ...
      {drawnPolygon && (
        <div className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-border bg-surface p-4">
          <div className="flex items-center gap-2 text-sm font-medium text-primary-dark"><Ruler className="h-4 w-4" /> {drawnPolygon.area.toLocaleString("es-ES")} ha</div>
          <input type="text" value={parcelName} onChange={(e) => setParcelName(e.target.value)} placeholder="Nombre de la parcela" className="flex-1 rounded-lg border border-border bg-app-bg px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
          <button onClick={handleSaveParcel} disabled={saving || !parcelName.trim()} className="cursor-pointer rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-primary-dark disabled:opacity-50">{saving ? "Guardando..." : "Guardar como parcela"}</button>
          <button onClick={() => setDrawnPolygon(null)} className="cursor-pointer rounded-lg px-3 py-2 text-sm text-muted-foreground transition-colors hover:text-destructive">Cancelar</button>
        </div>
      )}
                ...
                <Marker key={m.id} position={m.coords}>
                  <Popup>
                    <div className="min-w-[180px]">
                      ...
                      <Link to={`/parcels/${m.id}`} className="mt-2 inline-block text-xs font-medium text-primary hover:underline">Ver detalle →</Link>
                    </div>
                  </Popup>
                </Marker>
              ))}
