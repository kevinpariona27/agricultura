...
export function IrrigationForm({ initialValues, onSubmit, submitLabel, loading = false }: {
  ...
  return (<form onSubmit={handleSubmit} className="flex flex-col gap-5">
    ...
    <div><label className={lb}>Cantidad (litros) <span className="text-destructive ml-0.5">*</span></label><input type="number" required min="0.01" step="any" value={amount} onChange={e => setAmount(e.target.value)} className={ic} placeholder="Ej: 500" />{errors.amount && <p className={ec}>{errors.amount}</p>}</div>
    <div><label className={lb}>Fecha <span className="text-destructive ml-0.5">*</span></label><input type="date" required value={date} onChange={e => setDate(e.target.value)} className={ic} />{errors.irrigation_date && <p className={ec}>{errors.irrigation_date}</p>}</div>
    <div><label className={lb}>Método <span className="text-destructive ml-0.5">*</span></label><select required value={method} onChange={e => setMethod(e.target.value as IrrigationMethod)} className={ic}><option value="">Seleccionar método</option>{IRRIGATION_METHOD_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>{errors.method && <p className={ec}>{errors.method}</p>}</div>
