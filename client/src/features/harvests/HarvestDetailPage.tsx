...
export function HarvestDetailPage() {
  ...
  const unitLabel =
    HARVEST_UNIT_LABELS[current.unidad] ?? current.unidad;

  const fields = [
    { label: "Cultivo", value: cropVariety },
    {
      label: "Fecha de cosecha",
      value: new Date(current.fecha_cosecha).toLocaleDateString("es-AR"),
    },
    { label: "Cantidad", value: current.cantidad.toLocaleString() },
    { label: "Unidad", value: unitLabel },
    ...(current.rendimiento != null
      ? [{ label: "Rendimiento", value: current.rendimiento.toLocaleString() }]
      : []),
    ...(current.perdidas != null
      ? [{ label: "Pérdidas", value: current.perdidas.toLocaleString() }]
      : []),
    ...(current.notas
      ? [{ label: "Notas", value: current.notas }]
      : []),
    {
      label: "Creado",
    ...
    <div>
      ...
      <div className="mb-6 flex items-start justify-between">
        <h1 className="text-2xl font-semibold tracking-tight text-gray-900">
          Cosecha del{" "}
          {new Date(current.fecha_cosecha).toLocaleDateString("es-AR")}
        </h1>
        <div className="flex gap-2">
          <ProtectedAction roles={["admin", "manager"]}>
            <button
              onClick={() => navigate(`/harvests/${current.id}/edit`)}
