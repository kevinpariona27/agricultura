...
export function HarvestForm({ initialValues, onSubmit, submitLabel, loading = false }: {
  ...
  return (<form onSubmit={handleSubmit} className="flex flex-col gap-5">
    ...
    <div><label className={lb}>Cantidad</label><input type="number" required min="0.01" step="any" value={cantidad} onChange={e => setCantidad(e.target.value)} className={ic} placeholder="Ej: 500" />{errors.cantidad && <p className={ec}>{errors.cantidad}</p>}</div>
    <div><label className={lb}>Unidad</label><select required value={unidad} onChange={e => setUnidad(e.target.value as HarvestUnit)} className={ic}><option value="">Seleccionar unidad</option>{HARVEST_UNIT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>{errors.unidad && <p className={ec}>{errors.unidad}</p>}</div>
    <div><label className={lb}>Fecha de cosecha</label><input type="date" required value={date} onChange={e => setDate(e.target.value)} className={ic} />{errors.fecha_cosecha && <p className={ec}>{errors.fecha_cosecha}</p>}</div>
